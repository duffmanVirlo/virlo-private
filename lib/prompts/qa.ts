...
SPATIAL UNLOCK / POWER-ACCESS REASONING CHECK:
- If the product is an outlet extender, flat plug, extension splitter, power strip with flat plug, under-desk cord solution, or any product whose primary value is unlocking previously-unusable space/access, AND the script leads with feature-flex content (device count, total outlets, USB count, "plug X things in at once") instead of a spatial unlock story, flag as warning with component "dominant_angle" and issue "feature-flex angle for a spatial-unlock product — strongest conversion driver is usually the hidden outlet recovery / access expansion / furniture-flush unlock moment. Restructure: relatable spatial frustration → visible unlock movement on camera → secondary features layered after."
- For these product types, if no beat contains visible physical movement (plugging in on camera, cord routing shown, furniture moving, reveal of newly-accessible outlet), flag as warning with component "proof_movement" and issue "missing movement-based proof — spatial unlock products need visible transformation, not static device-count reveal. Add a beat where the creator performs the unlock on camera."
